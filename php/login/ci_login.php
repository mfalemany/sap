...
class ci_login extends toba_ci
{
	...
	function asignar_perfiles($nro_documento)
	{
		...
		$perfiles_condiciones = array(
			...
			'solicitante_subsidios' => array(
				array('clase' => 'co_personas','metodo' => 'es_docente',              'condicion' => "===TRUE"),
				array('clase' => 'co_personas','metodo' => 'tiene_mayor_dedicacion',  'condicion' => "===TRUE"),
				array('clase' => 'co_personas','metodo' => 'get_categoria_incentivos','condicion' => " == TRUE")	
			),
			'director_proyectos' => array(
		...
		foreach($perfiles_condiciones as $perfil => $condiciones){
			...
			if(!in_array($perfil,$perfiles_usuario)){
				...
				$corresponde = TRUE;
				//ei_arbol($perfiles_condiciones); die;
				...
				if($corresponde){
					try {
						toba::instancia()->vincular_usuario('sap',$nro_documento,$perfil);
